	...
	<script type="text/javascript">
		...
		function goInputPage(){
			...
			if(confirm("등록하시겠습니까?")){
				...
				//js에서 페이지 이동 명령어 a href=""와 동일한 역할 = location.href
			}
		}
	</script>
...
<%-- 
# 부서정보 검색 리스트 만들기
1. 검색 form 만들기 - dname, loc 검색할 수 있게 만들기
...
1. 프로세스 정리
	1) 화면에 등록 버튼을 클릭 등록 화면 이동
		<input type="button" value="등록" onclick="goInputPage()">
		function goInputPage(){
			if(confirm("등록하시겠습니까?")){
				location.href="a13_DeptInsertFrom.jsp";
				//js에서 페이지 이동 명령어 a href=""와 동일한 역할 = location.href
	...
	4) 요청값으로 dao에 등록 기능 메서드 호출
		...
		if(deptno!=null&& !deptno.equals("")){
			...
			A06_PareparedDao = new A06_PareparedDao();
			dao.insertDept(ins);
			isInsert = true;
		}
...
// 입력값 없이 submit버튼 클릭 시
// a01_begin/a12_DeptList/
//
String loc=request.getParameter("loc"); if(loc==null) loc="";
...
<form>
	<table>
		<col width="40%">
		<tr><th>부서명</th><td><input type="text" name="dname" vlaue="<%=dname %>"/></td></tr>
		<tr><th>부서위치</th><td><input type="text" name="loc" vlaue="<%=loc  %>"></td></tr>
		<tr><td colspan="2">
			<input type="button" value="등록" onclick="goInputPage()" style="background:tomato;"/>
