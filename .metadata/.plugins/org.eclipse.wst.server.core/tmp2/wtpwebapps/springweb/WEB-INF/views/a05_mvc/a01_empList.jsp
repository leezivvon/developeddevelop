...
<script type="text/javascript">
	$(document).ready(function(){
		//콤보박스는 스크립트단 처리해줘야 된대
		$("[name=deptno]").val("${sch.deptno}");	
		// document.querySelector("[name=deprno]").value="${sch.mgr}"(X)
		// jquery에서만 지원함
...
<div class="container">
  	...
  	<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
	    ...
	    <select name="job" class="form-control mr-sm-2">
	    	...
	    	<c:forEach var="job" items="${jobCom }">
	    		<option>${job }</option> <!-- value값 설정X >> 값이 똑같기때문에 -->
	    	</c:forEach>
	    </select>
   ...
   <table class="table table-hover table-striped">
    ...
    <tbody>
    	...
    	<tr>
    		<td>${emp.empno }</td><td>${emp.ename }</td><td>${emp.dname }</td>
