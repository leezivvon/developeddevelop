...
<script type="text/javascript">
	...
	function search(){
		$.ajax({
			...
			type:"post",
			data:$("#frm01").serialize(), // name="k" value="v" ==> 
			dataType:"json", //html, text default text
			success:function(data){
				// data : dataType이 설정되어 있지 않으면 text
				// 객체
				// d.addAttribute("dlist", @@@)
//				console.log(typeif(data)) //문자열string인지 객체object인지 // 객체여야한다
				var dlist = data.dlist
				var show=""
				// object일때 가능
				// dlsit.forEach(dlist, function(dept, idx){})
	...
	function goPage(deptno){
		$.ajax({
			...
			success:function(data){
				...
				// data.모델명
				// {dept:{deptno:10,...}}
				var dept=data.dept //"dept",service.getDept(deptno) 여기 dept
				$("#modal01").click() // 모달 로딩
