...
2. 동일 크기를 갖는 배열은 동일한 위치의 요소 데이터까지 연산 수행한다
    ...
    list03 = list01*2 ==> 각 요소들마다 데이터를 *2로 처리한다
                [2 ,4 ,6]
    list04 = list01+list02 ==> 각 요소의 같은 index위치에 데이터끼리 + 연산 처리 ==> [4 ,6, 8]
    

# ndarray의 비교연산자
1. 배열의 각 데이터를 비교하여 논리값을 저장하는 연산처리를 말한다
2. ex) boolList = np.equal(list01, list02)
