...
3. 초기화 파라미터 설정(web.xml)
	<context-param>
	 ...
	 공통위치에 키값으로 설정하고 그것만 변경하면 모든 소스를 자동적용되면 효과적으로 처리가 가능하다
	 web.xml에서 설정하여 변경함으로 공통적인 설정값에 대한 효과적인 처리를 할 수 있다
		
4. application 기본객체의 초기화 피라미터 관련 기능
	1) getInitParameter(String name) : 이름이 name인 웹 어플리케이션
		초기화 파리미터의 값을 읽어오다. 존재하지 않을 경우 null을 리턴한다
