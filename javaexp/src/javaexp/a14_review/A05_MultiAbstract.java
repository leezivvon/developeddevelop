...
//			p.partFun();
//		}
		// 어..? 나 ram을 추가하애되는데 ssd를 추가해야되는데 할 때 쉽게 추가 가능
		// = 부품을 기준으로 더 사양을 추가하여 처리할 수 있다 << 다형성이 강력한 이유
...
/*
# 1 대 다 관계 처리
1. 하나의 클래스 안에 여러 개의 클래스가 선언되는 것을 말한다
2. 단계별로 이해 
	...
	3) 1:다 관계 객체	
		class Mart{
			List<Product> p;
			Mart(){
				p = new ArrayList<Product>(); //생성자에 의해 초기
			}
	...
	4) 추상클래스에 대한 이해
		...
		ex) 삼성, 현대, LG ( 실제 회사 ) ==> 회사( 추상 )
			개, 당나귀, 말		==>  동물(추상)
		위 개념에 의해 추상클래스는 독립적으로 객체를 생성하지 못하지만 공통적인 기능과 재정의를 목적으로 선언한다
		abstract class Company{
			public void working(){ // 공통메서드(공통으로 사용하는 거 선언)
				회사가 일을 하다
			}
		...
		다형성 처리를 통해 상위 클래스가 여러 형태의 다양한 하위 객체를 사용한다
		com1.working();		공통 기능메서드
		com1.earnMoney();	다양한 하위 기능메서드(재정의된 메서드 사용)
	5) 1: 다 관계 추상 클래스에서 상속받은 하위 클래스 선언
		



# 추상클래스를 상속받은 하위 클래스의 내용을 다형성으로 처리하여 1:다 관계를 처리하는 경우
1. 예제
	컴퓨터의 여러가지 부품을 Part라는 추상클래스를 상속받은 하위실제 클래스로 생성하여 
	다양한 형태를 실제객체를 할당할 수 있는 구조로 처리하는 1:다 구조를 말한다
2. 처리 순서
	1) 추상 클래스 선언
	2) 상속받은 하위클래스 선언 : 필요시 추가적으로 상속받은 하위클래스를 정의하여 처리하여야 한다
...
abstract class ComPart {
	...
	// 하위에서 이 생성자를 통해 필드를 초기화 할수 있게 선언
	...
	// 		==> 메서드를 통일화할 수 있다
	//		==> 다양한 형태로 기능을 처리할 수 있다(다형성 - polymorphism)		
	public abstract void partFun();
}

class Cpu extends ComPart{	
	public Cpu(String perFrm) {
		// 상속을 하면 상속된 하위클래스는 상위클래스의 생성자를 반드시 호출하여야 한다
		// super(매개변수1, 매개변수2)
		// Compart(String kind, string perFrm)
		super("CPU", perFrm);
	}
	//abstract로 선언된 추상메서드는 반드시 재정의하여 선언해야 한다
	// 추상메서드 => 재정의 => 다형성(하위에 같은 기능메서드명으로 다양한 기능 처리가 가능)
	@Override
	public void partFun() {
...
class Ram extends ComPart{
	...
	public void partFun() {
		System.out.println(getKind()+"는 "+getPerFrm()+"성능을 가지고 있다.");
		System.out.println("컴퓨터에서 주로 휘발성 메모리를 해주는 부품이다");
	}	
}
class Ssd extends ComPart{
	...
	public void partFun() {
		System.out.println(getKind()+"는 "+getPerFrm()+"성능을 가지고 있다.");
		System.out.println("컴퓨터에서 하드웨어적으로 데이터를 영구 저장해주는 저장메모리 부품이다");
	}	
}
