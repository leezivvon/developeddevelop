...
public class A01_Begin {
	...
	public static void main(String[] args) {
		...
		1. 자바에서 기본적으로 제공하는 라이브러리
		   프로그램 개발ㅇ 자주 사용되는 클래스 및 인터페이스 모음
		2. API 도큐먼트
		   - 쉽게 api 찾아 이용할 수 있도록 문서화 한 것
		...
		3. 주요 클래스
		   Object - 자바 클래스의 최상위 클래스로 사용
		            모든 클래스(내장/사용자정의)는 Object를 상속하고 있다
		            그러므로 Object가 가지고 있는 기본적인 메서드는 extends안하고 재정의하거나 사용할 수 있다
		            toString(),  hashCode()
		            1) ArrayList list => ArrayList<Object> list
		            2) 다형성에 의해서 모든 객체는 Object객체에 할당이 가능하다
		            	Object ob = new Person()
		            3) 그래서 상위에 재정의하는 메서드는 상위 Object의 참조변수로 사용가능하지만
		            4) 하위 고유의 메서드를 사용하고자 할 때는 typecasting이 필요로 하다 	
		               ex)
		               Person클래스에서만 선언된 name 필드와 getName()메서드를 상위 클래스의 참조 변수 ob로 사용 불가,
		               ob.getName() ( X )
		               타입 캐스팅을 하여야 한다
		               Person p = (Person) ob;
		               p.getName() ( O )		    
		            5) ArrayList list = new ArrayList();
		               ...
		               list.add("안녕");
		               list.add(new Man());
		               이렇게 모든 객체를 할당할 수 있지만 해당 객체의 고유의 필드와 메서드를 사용하려면 타입캐스팅이 필요하다
		               Person p = (Person)list.get(0);
		               Man m = (Man)list.get(1);
		            6) 위 같이 할당은 가능하지만 사용하려는 객체의 유형으로 타입캐스팅하는 불편함이 있는데 
		               단일 객체를 사용할 경우 Generic(<객체유형>)을 선언하면 사용 가능하다
		   ...
		   System - 표준 입력(키보드)으로부터 데이터를 입력 받는 표준 출력장치(모니터)로 출력하기 위해 사용
		            System.in
		   			자바 가상머신을 종료시킬 때 사용
		   			System.out
		   ...
		   String - 문자열을 저장하고 여러가지 정보를 얻을 때
		            "문자열"+"문자열" : 메모리 2개
		   StrinBuffer, StringBuilder  - 문자열을 저장하고 내부 문자열을 조작할 때 사용
		                                 sbf.append("문자열")
...
class Member{
	...
	public boolean equals(Object obj) { // Object obj = new Member("홍길동")
		...
		Member m = (Member)obj; 
		// 해당객체의 속성을 사용하려면 type casting이 필요하다
		// 문자열로 name속성이 같을 때 true 그 외는 false
