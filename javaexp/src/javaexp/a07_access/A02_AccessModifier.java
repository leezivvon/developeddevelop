...
public class A02_AccessModifier {
	...
	public static void main(String[] args) {
		...
		*/
		// 클래스 접근 제어자 확인..
		// 1. product클래스 붙여넣기
//		prodct 클래스 붙여넣기; // 같은 패키지에 있는 클래스 호출 가능
		                    // 같은 패키지는 생성자가 default 접근제어자 여도 접근이 가능
//		System.out.println(prod.name);
		
		A01_Person pson; // 다른 패키지에 있는 클래스 호출 
//		A02_Music music; // 다른 패키지에 있는 클래스가 public이 아닐 때 에러가 발생
		pson = new A01_Person(); 
		A01_Person pson2;
//		pson2 = new A01_Person("홍길동");  생성자가 default 접근제어자이기에 에러발생
		/*
		# 패키지에 따른 클래스와 생성자의 접근 제한
		1. 호출하는 클래스가 같은 패키지인 경우 접근제어자가 public, X(default)경우 자유롭게 클래스 선언과 생성자 선언이 가능하다
		2. 호출하는 클래스가 다른 패키지인 경우 접근제어자가 public인 경우에 class선언이 가능하고 생성자도 public인 경우에 생성이 가능하다
		3. 같은 패키지더라도 접근제어자가 private인 경우에는 외부 클래스에서 접근이 불가능하다
