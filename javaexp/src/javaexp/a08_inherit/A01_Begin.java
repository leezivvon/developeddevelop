...
public class A01_Begin {
	...
	public static void main(String[] args) {
		...
		# 상속(intheritance) 개념과 활용    
		1. 상속의 효과
		    1) 부모 클래스를 재사용해 자식 클래스 빨리 개발 가능
		    2) 반복된 코드 중복을 줄임
		    3) 유지 보수 편리성 제공
		    4) 객체 다형성 구현 가능
		2. 상속 대상 제한
		    1) 부모 클래스의 private 접근 갖는 필드와 메소드 제외
		    2) 부모 클래스가 다른 패키지에 있을 경우, default접근 갖는 필드와 메소드도 제외 
		    ps) super()생성자나 public 기능 메서드를 통해서 간접적으로 필드의 값을 할당하거나 호출할 수 있다
		    3) protected : 상속관계 있고 해당 접근제어자로 선언되어 있으면 접근이 가능하다(필드/메서드)
		# extends 키워드
		1. 자식 클래스가 상속할 부모 클래스를 지정하는 키워드
		    class Child extends Parent{}
		2. 자바는 단일 상속 - 부모 클래스를 여러개 나열 불가능하다
...
//		d1.cooking();
		...
		Son s1 = new Son();
		System.out.println("< 아들의 특징 >");
		s1.showAll();
	}
}
//ex 아버지(모습이 통통하다, running() 빠른 속도로 뛴다), 아들(키가 크다, showAll()-부모구성+키)
class Father {
...
//		System.out.println(성격: "+charact);   접근제어자가 private인 경우 처리하지 못함
		System.out.println("성격: "+shape);   //간접적으로 접근을 허용
	}
}
...
class Daughter extends Mother{
	...
	void callTalent( ) {
		...
		System.out.println(charac);// 부모의 속성   // Mother class의 변수 그대로 사용 가능
		cooking(); // 부모의 메서드
