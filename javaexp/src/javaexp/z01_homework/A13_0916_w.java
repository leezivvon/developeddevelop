...
[1단계:개념] 10. protected의 접근 범위에 대하여 예제를 통하여 기술하세요		
		...
		// class Mart 생성 없이 하는 .. 
		// public void buyInfo() 도 필요 없고
		ArrayList<Productt> buyList = new ArrayList<Productt>();
		buyList.add(new Productt("콘칩",2380,1));
		buyList.add(new Productt("빈대떡",10000,2));
		buyList.add(new Productt("해창막걸리",9000,1));
		int sum=0; // 누적할 변수는 for문안에 들어가면 안됨 = 상단에 전역변수
		for(int idx=0;idx<buyList.size();idx++) {
			Productt p = buyList.get(idx);
			System.out.print(p.getPname()+"\t");
			System.out.print(p.getPprice()+"\t");
			System.out.print(p.getPcnt()+"\t");
			int tot = p.getPprice()*p.getPcnt();
		...
		*/
		...
		pf.addStu(new Students("박은빈", 13, "심리학과"));
		pf.addStu(new Students("최우식", 15, "경제학과"));
		pf.showAddStuList();
		//7-2
		// 한 번 배운 것들을 이용해서 랜덤으로 좌석 배정을 해보고 싶었는데 
		// 아직 배운게 부족한지 실력이 부족한지 잘 안되네요..ㅎㅎㅎㅎ
		Train t = new Train("부산행 KTX 764호차");
		String fnum="";
		for(int idx=65;idx<77;idx++) {
			char anum =(char)((int)Math.random()*12+idx);
			for(int cnt=49; cnt<58;cnt++) {
				int num = (int)Math.random()*9+idx;
				fnum = anum+Integer.toString(num);
			}
		}
		t.addPassenger(new Seat("Strawberry", fnum));
		t.showAddPas();		
		
		//8,9
		Robot r01 = new Dagan();
		Robot r02 = new Gundam();
		r01.attack();
		r02.attack();

		//10
		/*
		 접근제어자 protected는 다른 package임에도 상속관계라면 접근이 가능하게끔 한다.
   >> 피상속..		 
...
public class Son1 extends WoodCutter { 
	...
	public void callWoodCutter() {
		...
		다른 패키지일지라도 protected라고 붙은 접근제어자의 변수는 상속관계일 때는 접근이 가능하다
		System.out.println("protected: "+inheritMoney);
		
		★상속관계임에도 불구하고 참조값으로 외부 객체를 호출하는 형식으로 처리할 때는 protected로 접근 할 수 없다★
...
//8,9
/*
상속 관계에서 생성자의 하위 객체는 상위 객체의 생성자를 생성하면서 처리가 된다.
default 생성자에 의해서 기본적으로 호출이 되고 매개변수가 있는 생성자가 생긴 순간 
하위 객체에서는 반드시 호출해야주어야 하고 이때 super()을 사용한다. 
*/
abstract class Robot{
	private String kind;
	public Robot(String kind) { // 상속관계의 상위 객체의 생성자에 String 매개변수 하나 
		this.kind = kind;
	}
...
class Dagan extends Robot{
	public Dagan() {
		super("다간"); // 상속관계의 하위 객체의 생성자에서 super()로 String 매개변수 하나 받음
	}
	public void attack() {
		System.out.println(getKind()+"!! 필살기 사용!!!");
	}	
}
class Gundam extends Robot{
	public Gundam() {
		super("건담"); // 상속관계의 하위 객체의 생성자에서 super()로 String 매개변수 하나 받음
	}
	public void attack() {
		System.out.println(getKind()+"!! 필살기 사용!!!");
	}	
...
class Train {
	...
	public void showAddPas() {
		System.out.println("__"+this.trainNum+" 기차표 예매 목록__");
		if(seatList.size()>0) {
			for(Seat s :seatList) {
				s.showSeat();
			}
		}else {
			System.out.println("예매한 좌석이 없습니다.");
		}
	}
}
class Seat {
	private String id;
	private String seat;
	public Seat(String id, String seat) {
		this.id = id;
		this.seat = seat;
	}
	public void showSeat() {
		System.out.println("\t"+this.id+" | "+this.seat);
	}
	public String getId() {
		return id;
	}
