...
2. 단, 저장을 누르는 순간 연결은 끊어진다
	-> 파일에 변경된 내용은 마지막에 저장을 하든지
	-> 중요한 부분에서 저장을 한 번 하고 위 연결옵션을 다시 선택
*/
 ...
 */
SELECT empno, empno AS NO, ename AS name,  -- AS 사용
      hiredate birthdate, deptno dno       -- 공백 사용
FROM emp;
/*
...
/*
 # 기본적인 테이블 처리 문법은 여러 테이블과 함께 사용할 때, 활용된다. 
   동일한 컬럼이 있을 경우 구분하기 위한 내용이다. 테이블이 하나밖에 없을 때는 테이블명을 생략해도 된다.
 */
SELECT * 
FROM emp;
...
-- 그런데, 특정 컬럼을 나타내고 전체 컬럼을 나타낼 때는 alias를 하나의 테이블인 경우도 처리해야 한다.
SELECT empno NO, e.*
FROM emp e;
--SELECT empno NO, *
--FROM emp;   -- 에러발생.
...
/*
 # alias의 공백과 특수문자 사용
 1. alias명으로 공백이나 특수문자를 사용할 때는 ""로 양옆에 쌍따옴표를 처리한다.
 2. 한글이라도 붙여서 처리하는 경우에는 필요 없다.
 */
SELECT ename "사 원 정 보", ename "@ 사원 정보 #", sal 급여
FROM emp;

-- ex) 해당 컬럼이 alias로 처리하여 출력하세요.
--      empno("사 원 번 호") ename("!!@사원명@!!") sal("급!!!여")
...
SELECT empno, empno + 10 "사원번호+10",
  	   ...
  	   sal*(deptno/10) "급여와 부서의 연산 ",
  	   mod(sal, deptno) "나머지"    -- 나머지값 처리
FROM emp;
...
SELECT empno + 10000 "사원번호(만단위)",
       ...
       sal+nvl(comm,0) "합산2(sal+comm)" -- < NULL인 데이터를 0으로 default한다는 의미
FROM emp;
