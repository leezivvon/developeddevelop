...
2. sql 함수의 유형
	1) 단일 행 함수 : 테이블에 저장되어 있는 개별 행을 대상으로 하는 함수를 적용하여 하나의 결과를 반환하는 함수이다
		ex) 대문자를 소문자로 변환 처리
	2) 복수 행 함수 : 조건에 따라 여러 행을 그룹화하여 그룹별로 결과를 하나씩 변환하는 함수
		         	여러 행의 데이터를 함수를 적용했을 때 단이 또는 줄어든 결과를 리턴하여 처리할 때 사용된다
		         	ex) 부서별 최소급여
		         	10  3000
		         	10  3500    deptno, min(sal)   ==> 10, 2000    // group by deptno 
		         	10  2000                           20, 1000
		         	20  1000
		         	20  1200
		         	            min(sal)  ==> 1000                 // 전체 데이터
...
4. 단일행 함수의 사용법
	select 함수명(칼럼명/데이터, 매개변수1, 매개변수2...)
...
2. 'a'문자로 입력하거나 'A'를 입력해서 검색하더라도 대소문자 상관없이 처리가 필요할 때 위 함수를 처리한다
*/
...
-- ex) 사원정보 테이블에 직책이 'Man'|'MAN'|'man'등 여러형식으로 있을지라도 
--     대소문자 관계없이 키워드 검색되게 하려면 사용하는 sql을 처리하세요
SELECT *
FROM emp
WHERE lower(job) LIKE '%'||lower('Man')||'%';
--모두 다 lower로 사용하면 검색할 수 있다
/*
# 문자열 길이 반환
1. length(데이터/컬럼명) : 입력되는 문자열의 길이(글자수)를 반환하는 함수
2. lengthb(데이터/컬럼명) : 입력되는 문자열의 바이트를 반환하는 함수
	한글이나 특수문자는 1글자가 3byte인 경우가 있다. 영어와 한글은 글자 수에서 차이가 난다
	
	cf) dual : 오라클에서 지원되는 임시가상테이블로 테스트 용으로 한 라인에 데이터와 함수의 적용결과를 확인할 수 있다
*/
SELECT * FROM dual;
SELECT 15 num01, 16 num02, 15+16 sum01 FROM dual;
SELECT '안녕' strl, '반가워' str2 FROM dual;
SELECT 'hello' 인사1, length('hello') 인사1크기1, length('hello') 인사1크기2,
	   '안녕' 인사2, length('안녕') 인사2크기1, lengthb('안녕') 인사2크기2 -- 영문은 글자수랑 byte수랑 같지만 한글은 아니야
FROM dual;	   

SELECT ename, length(ename) 사원명길이
FROM emp;

-- ex) 사원 테이블에서 사원명과 직책의 글자 수를 표현하되 직책이 6 이상인 데이터만 출력하세요
SELECT ename, LENGTH(ename) 사원명글자수, job, length(job) 직책글자수
FROM emp
WHERE length(job)>=6;
...
2. substr(문자열데이터, 시작위치, 시작위치의 개수) :
	...
	ex) 코드성 데이터 - 사번, 주민번호, 학번에는 각각의 위치별로 의미하는 바가 있기 때문에 추출하여 데이터 활용한다
		주민번호의 경우 생년월일을 위치에 따라 추출할 수 있다
			
*/
SELECT empno, ename, job, concat(ename, job) "이름과 직책"
FROM emp;
SELECT substr('sql*plus', 5, 4) "데이터" FROM dual;   --plus출력

SELECT * FROM emp;
--ex) 사원명과 사원번호를 이어서 표현하고 직책은 2번째부터 3글자를 추출해서 출력하세요
SELECT concat(ename, empno) "사원명/사원번호", substr(job, 2, 3) "직책일부" 
FROM emp;
